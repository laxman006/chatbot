...
export default function ChatSidebar({
  ...
  useEffect(() => {
    async function checkAccess() {
      try {
        ...
        console.error('Failed to check API research access:', error);
      }
    }
  ...
  // Toggle API Research handler
  const handleToggleApiResearch = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setApiResearchAccess(prev => ({
          ...prev,
          enabled: data.enabled
        }));
      }
    } catch (error) {
      console.error('Failed to toggle API research:', error);
    } finally {
      setIsTogglingApiResearch(false);
    }
  };
    ...
    <aside className={`chatgpt-sidebar ${isOpen ? 'open' : 'closed'}`}>
      <div className="sidebar-header">
        <div className="sidebar-top-row">
          <button
            className="mobile-sidebar-toggle-inside"
            onClick={onToggle}
            title="Close sidebar"
                  ...
                  >
                    ...
                    <div 
                      ...
                      onMouseDown={(e) => {
                        ...
                        e.stopPropagation();
                        e.nativeEvent.stopImmediatePropagation();
                        console.log('[Admin Nav] FAQ Upload clicked - mousedown');
                        handleAdminNavigation('/admin/faq-upload', e);
                      }}
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        e.nativeEvent.stopImmediatePropagation();
                        console.log('[Admin Nav] FAQ Upload clicked - click');
                        handleAdminNavigation('/admin/faq-upload', e);
                      }}
                      style={{ cursor: 'pointer', pointerEvents: 'auto', position: 'relative', zIndex: 10000 }}
                    >
