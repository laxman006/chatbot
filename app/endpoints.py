...
def apply_section_based_reranking(
    ...
    # Source priority boosts for internal sources
    SOURCE_BOOSTS = {
        "jira": 0.10,           # Boost Jira tickets
        "sharepoint": 0.08,      # Boost SharePoint docs
        "pdfs": 0.05,            # Boost PDFs
        "blog": -0.15,           # Penalize blog posts
    }
    
    # Detect if this is a troubleshooting query
    troubleshooting_keywords = [
        "error", "fix", "issue", "problem", "bug", "failed", "resolve", 
        "troubleshoot", "solution", "workaround", "retry", "broken",
        "not working", "doesn't work", "how to fix", "how to resolve"
    ]
    is_troubleshooting = any(keyword in query.lower() for keyword in troubleshooting_keywords)
    
    # Get base reranker scores
    reranked_base = cross_reranker.rerank(query, candidates, top_k=len(candidates))
    
    # Apply section-based and source-based boosting
    boosted_results = []
    jira_docs = []
    sharepoint_docs = []
    ...
    for doc, base_score in reranked_base:
        ...
        if "jira" in source_type or "jira" in tag:
            source_boost = SOURCE_BOOSTS.get("jira", 0.0)
            jira_docs.append((doc, base_score + section_boost + source_boost))
        elif "sharepoint" in source_type or "sharepoint" in tag:
            source_boost = SOURCE_BOOSTS.get("sharepoint", 0.0)
            sharepoint_docs.append((doc, base_score + section_boost + source_boost))
        elif "pdf" in source_type or "pdf" in tag:
            source_boost = SOURCE_BOOSTS.get("pdfs", 0.0)
        elif "blog" in source_type or "blog" in tag or ("web" in source_type and "jira" not in tag):
            source_boost = SOURCE_BOOSTS.get("blog", 0.0)
        
        # Apply boosts
        boosted_score = base_score + section_boost + source_boost
        
        boosted_results.append((doc, boosted_score))
        if section in SECTION_BOOSTS or source_boost != 0.0:
            print(f"[RERANK] Section '{section}' ({section_priority}): base={base_score:.4f}, section_boost={section_boost:.4f}, source_boost={source_boost:.4f}, final={boosted_score:.4f}")
